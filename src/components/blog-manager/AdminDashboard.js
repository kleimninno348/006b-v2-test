...
const GlobalStyle = () => (
  <style dangerouslySetInnerHTML={{__html: `
    ...
    .sparkle-btn-wrap { position: relative; display: inline-flex; }
    .block-view-toggle .sparkle-button { --active: 0; --play-state: paused; --cut: 0.1em; --spark: 1.6s; --transition: 0.25s; --bg: radial-gradient(40% 50% at center 100%, hsl(270 calc(var(--active) * 97%) 72% / var(--active)), transparent), radial-gradient(80% 100% at center 120%, hsl(260 calc(var(--active) * 97%) 70% / var(--active)), transparent), hsl(260 calc(var(--active) * 97%) calc((var(--active) * 44%) + 12%)); background: var(--bg); font-size: 0.82rem; font-weight: 600; border: 0; cursor: pointer; padding: 0.55em 1.05em; display: flex; align-items: center; gap: 0.25em; white-space: nowrap; border-radius: 100px; position: relative; z-index: 1; box-shadow: 0 0 calc(var(--active) * 3em) calc(var(--active) * 1em) hsl(260 97% 61% / 0.75), 0 0 0 0 hsl(260 calc(var(--active) * 97%) calc((var(--active) * 50%) + 30%)) inset, 0 -0.05em 0 0 hsl(260 calc(var(--active) * 97%) calc(var(--active) * 60%)) inset; transition: box-shadow 0.3s, scale 0.3s, background 0.3s; scale: calc(1 + (var(--active) * 0.08)); font-family: inherit; }
    .block-view-toggle .sparkle-button:active { scale: 1; }
    .block-view-toggle .sparkle-button.is-selected, .block-view-toggle .sparkle-button:is(:hover, :focus-visible) { --active: 1; --play-state: running; outline: none; }
    .block-view-toggle .sparkle-button:before { content: ""; position: absolute; inset: -0.2em; z-index: -1; border: 0.2em solid hsl(260 97% 50% / 0.5); border-radius: 100px; opacity: var(--active, 0); transition: opacity var(--transition); }
    .block-view-toggle .spark { position: absolute; inset: 0; border-radius: 100px; rotate: 0deg; overflow: hidden; mask: linear-gradient(white, transparent 50%); animation: sparkle-flip calc(var(--spark) * 2) infinite steps(2, end); pointer-events: none; }
    @keyframes sparkle-flip { to { rotate: 360deg; } }
    .block-view-toggle .spark:before { content: ""; position: absolute; width: 200%; aspect-ratio: 1; top: 0%; left: 50%; z-index: -1; translate: -50% -15%; transform: rotate(-90deg); opacity: calc((var(--active)) + 0.35); background: conic-gradient(from 0deg, transparent 0 340deg, white 360deg); transition: opacity var(--transition); animation: sparkle-rotate var(--spark) linear infinite both; }
    @keyframes sparkle-rotate { to { transform: rotate(90deg); } }
    .block-view-toggle .spark:after { content: ""; position: absolute; inset: var(--cut); border-radius: 100px; }
    .block-view-toggle .backdrop { position: absolute; inset: var(--cut); background: var(--bg); border-radius: 100px; transition: background var(--transition); pointer-events: none; }
    .block-view-toggle .sparkle { inline-size: 1.15em; translate: -18% -5%; flex-shrink: 0; pointer-events: none; }
    .block-view-toggle .sparkle path { color: hsl(0 0% calc((var(--active, 0) * 70%) + var(--base))); transform-box: fill-box; transform-origin: center; fill: currentColor; stroke: currentColor; animation-duration: 0.6s; transition: color var(--transition); }
    .block-view-toggle .sparkle-button:is(:hover, :focus-visible, .is-selected) .sparkle path { animation-name: sparkle-bounce; }
    @keyframes sparkle-bounce { 35%, 65% { scale: var(--scale); } }
    .block-view-toggle .sparkle path:nth-of-type(1) { --scale: 0.5; --delay: 0.1; --base: 40%; animation-delay: calc((var(--transition) * 1.5) + (var(--delay) * 1s)); }
    .block-view-toggle .sparkle path:nth-of-type(2) { --scale: 1.5; --delay: 0.2; --base: 20%; animation-delay: calc((var(--transition) * 1.5) + (var(--delay) * 1s)); }
    .block-view-toggle .sparkle path:nth-of-type(3) { --scale: 2.5; --delay: 0.35; --base: 30%; animation-delay: calc((var(--transition) * 1.5) + (var(--delay) * 1s)); }
    .block-view-toggle .sparkle-btn-text { translate: 2% -6%; letter-spacing: 0.02em; background: linear-gradient(90deg, hsl(0 0% calc((var(--active) * 100%) + 65%)), hsl(0 0% calc((var(--active) * 100%) + 26%))); -webkit-background-clip: text; background-clip: text; color: transparent; transition: background var(--transition); pointer-events: none; }
    .block-view-toggle .particle-pen { position: absolute; width: 200%; aspect-ratio: 1; top: 50%; left: 50%; translate: -50% -50%; -webkit-mask: radial-gradient(white, transparent 65%); z-index: 0; opacity: var(--active, 0); transition: opacity var(--transition); pointer-events: none; }
    .block-view-toggle .sparkle-button.is-selected ~ .particle-pen, .block-view-toggle .sparkle-button:is(:hover, :focus-visible) ~ .particle-pen { --active: 1; --play-state: running; opacity: 1; }
    .block-view-toggle .particle { fill: white; width: calc(var(--size, 0.25) * 1rem); aspect-ratio: 1; position: absolute; top: calc(var(--y) * 1%); left: calc(var(--x) * 1%); opacity: var(--alpha, 1); animation: sparkle-float-out calc(var(--duration, 1) * 1s) calc(var(--delay) * -1s) infinite linear; transform-origin: var(--origin-x, 1000%) var(--origin-y, 1000%); animation-play-state: var(--play-state, paused); }
    .block-view-toggle .particle path { fill: hsl(0 0% 90%); stroke: none; }
    .block-view-toggle .particle:nth-of-type(even) { animation-direction: reverse; }
    @keyframes sparkle-float-out { to { rotate: 360deg; } }
...
const SparkleViewButton = ({ label, active, onClick }) => {
  const particles = useMemo(() => (
    Array.from({ length: 10 }, (_, i) => ({
      ...
      style: {
        ...
        '--delay': `${Math.random() * 2}`,
        '--origin-x': `${Math.random() > 0.5 ? 120 : -120}%`,
        '--origin-y': `${Math.random() > 0.5 ? 120 : -120}%`,
  ...
  return (
    <div className="sparkle-btn-wrap">
      <button
        type="button"
        className={`sparkle-button ${active ? 'is-selected' : ''}`}
        onClick={onClick}
        aria-pressed={active}
      >
        ...
        <span className="sparkle-btn-text">{label}</span>
      </button>
      <span className="particle-pen" aria-hidden>
        {particles.map((p) => (
          <svg key={p.id} className="particle" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" style={p.style}>
            <path d={SPARKLE_PARTICLE_PATH} />
          </svg>
        ))}
      </span>
    </div>
